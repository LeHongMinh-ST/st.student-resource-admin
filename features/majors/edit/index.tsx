...
const MajorUpdatePage = () => {
  ...
  const { updateMajor, getMajor } = useMajorService();

  const { query } = useRouter();
  const { id } = query;

  const { data, isLoading } = useSWR([id], () =>
    getMajor(id as string)
      .then((res) => res.data)
      .catch((e) => e)
  );
  useEffect(() => {
  ...
  const onSubmit = async (data: Major) => {
    if (!isSubmitting) {
      try {
        const res = await updateMajor(data);
        if (res) {
          notifications.show({
            title: 'Thành công!',
  ...
  return (
    <MajorUpdatePageStyled>
      <Container fluid>
        <Stack gap="lg">
          <Skeleton visible={isLoading}>
            <PageHeader
              title={`Chuyên ngành - Chỉnh sửa - #${id}`}
              breadcrumbItems={[
                { title: 'Bảng điều khiển', href: dashboardRoute.dashboard },
                { title: 'Chuyên ngành', href: departmentRoute.list },
            ...
            <Grid gutter={{ base: 'lg', lg: 'xl' }}>
              <Grid.Col span={{ base: 12, md: 8 }}>
                <Surface component={Paper} p="md" shadow="md" radius="md" h="100%">
                  <Stack gap={32}>
                    <Fieldset legend="Thông tin chuyên ngành">
                      <Stack>
                        <Skeleton visible={isLoading}>
                          <TextInput
                            withAsterisk
...
const MajorUpdatePageStyled = styled.div``;
